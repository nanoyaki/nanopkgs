<!--
SPDX-FileCopyrightText: 2025 Hana Kretzer <hanakretzer@gmail.com>

SPDX-License-Identifier: CC0-1.0
-->

# Nanopkgs

Nightly package updates and non-merged nixpkgs PRs for software I use.

## Available packages

- `avdump` -> `B9005-GitHubRelease-unstable-2024-03-10`
- `check-project-conflicts`
- `clipboard-manager` -> `0.1.0-unstable-2026-02-15`
- `cosmic-classic-menu` -> `0.0.13-unstable-2026-03-09`
- `cosmic-ext-applet-privacy-indicator` -> `0.2.0-unstable-2026-02-20`
- `dwproton-bin` -> `10.0-21`
- `error-pages` -> `0-unstable-2024-11-22`
- `fireshare` -> `1.5.1-unstable-2026-03-15`
- `flaresolverr` -> `3.4.6`
- `jellyfin-web-with-plugins`
- `meow` -> `2.1.5`
- `midnight-theme` -> `0-unstable-2026-03-16`
- `oldlibcosmicAppHook`
- `openrgb` -> `release_candidate_1.0rc2-unstable-2026-03-17`
- `osu-lazer-bin` -> `2026.318.0-tachyon`
- `prefetch`
- `prowlarr` -> `2.3.4.5307`
- `pyon` -> `0.1.0`
- `rofi-themes` -> `0-unstable-2025-07-26`
- `shoko` -> `6.0.0-dev.45-unstable-2026-03-20`
- `shoko-webui` -> `2.6.0-dev.16-unstable-2026-03-20`
- `shokofin` -> `6.0.4-dev.3-unstable-2026-02-26`
- `startvrc`
- `suwayomi-server` -> `2.1.1867-unstable-2026-02-21`
- `suwayomi-webui` -> `20251230.01-unstable-2026-03-20`
- `whisparr` -> `3.1.0.2116`
- `wkeys` -> `0.1.2-unstable-2026-02-16`
- `writeSystemdToggle`
- `zigbee2mqtt` -> `2.9.1-unstable-2026-03-02`
