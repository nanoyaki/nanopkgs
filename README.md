<!--
SPDX-FileCopyrightText: 2025 Hana Kretzer <hanakretzer@gmail.com>

SPDX-License-Identifier: CC0-1.0
-->

# Nanopkgs

Nightly package updates and non-merged nixpkgs PRs for software I use.

## Available packages

- `avdump` -> `B9005-GitHubRelease-unstable-2024-03-10`
- `check-project-conflicts`
- `clipboard-manager` -> `0.1.0-unstable-2026-03-24`
- `cosmic-classic-menu` -> `0.0.13-unstable-2026-03-09`
- `cosmic-ext-applet-privacy-indicator` -> `0.2.0-unstable-2026-02-20`
- `dwproton-bin` -> `10.0-23`
- `error-pages` -> `0-unstable-2024-11-22`
- `flaresolverr` -> `3.4.6`
- `jellyfin-web-with-plugins`
- `meow` -> `2.1.5`
- `midnight-theme` -> `0-unstable-2026-04-08`
- `openrgb` -> `release_candidate_1.0rc2-unstable-2026-04-13`
- `osu-lazer-bin` -> `2026.408.0-tachyon`
- `prefetch`
- `prowlarr` -> `2.3.6.5351`
- `pyon` -> `0.1.0`
- `rofi-themes` -> `0-unstable-2025-07-26`
- `shoko` -> `6.0.0-dev.59-unstable-2026-04-13`
- `shoko-webui` -> `2.6.0-dev.21-unstable-2026-04-12`
- `shokofin` -> `6.0.5-dev.7-unstable-2026-04-11`
- `startvrc`
- `suwayomi-server` -> `2.1.1867-unstable-2026-03-31`
- `suwayomi-webui` -> `20251230.01-unstable-2026-04-13`
- `whisparr` -> `3.1.0.2116`
- `wkeys` -> `0.1.2-unstable-2026-02-16`
- `writeSystemdToggle`
- `zigbee2mqtt` -> `2.9.2-unstable-2026-04-01`
